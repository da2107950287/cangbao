<template>
  <div class="container">
        ...
        <el-tab-pane label="圈子动态" name="second">
          ...
          <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
            ...
            <el-table-column prop="" label="发布者头像" align="center">
              <template slot-scope="scope">
                <el-image :src="scope.row.headportrait" style="width: 60px;"></el-image>
              </template>
            </el-table-column>
            <el-table-column prop="" label="发布图片/视频" align="center" width="200">
              <template slot-scope="scope">

                <video v-if="scope.row.dyType==1" :src="scope.row.picture" controls
                  style="width: 180px;height: 100px;"></video>
                <el-image v-else-if="scope.row.dyType==2" :src="scope.row.picture" style="width: 180px;height: 100px;">
                </el-image>
              </template>
            </el-table-column>
    ...
    <div v-if="type==3">
        ...
        <div style="display: flex;">
          ...
          <div style="width: 100%;">
            ...
            <div v-if="dynamicInfo.dyType==1">
              <video v-for="(item,index) in dynamicInfo.picture" :key="index" :src="item"></video>
            </div>
            <div v-else>
              <el-image style="margin:5px;width: 200px;height: 100px;" v-for="(item,index) in dynamicInfo.picture"
                :key="index" :src="item"></el-image>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style="display: flex;margin-top: 10px;" v-for="(el,index) in oneComment" :key="index">
        ...
        <div style="width: 100%;">
          ...
          <div v-if="el.dyType==1">
            <video v-for="(item,index) in el.picture" :key="index" :src="item"></video>
          </div>
          <div v-else>
            <el-image style="margin:5px;width: 200px;height: 100px;" v-for="(item,index) in el.picture" :key="index"
              :src="item"></el-image>
          </div>
      ...
      <!-- <el-tabs v-model="activeDy" type="card" @tab-click="handleClick">
        <el-tab-pane label="动态详情" name="dy1">
          <dynamic-form :form="form" :dictionaryList="dictionaryList" @saveEdit="saveEdit"></dynamic-form>
        </el-tab-pane>
        <el-tab-pane label="动态评论" name="dy2">
          <div class="handle-box">
            <span>状态：</span>
            <el-select v-model="state" placeholder="请选择类型" class="handle-search mr10">
              <el-option v-for="(item,index) in statesList" :key="index" :label="item.name" :value="item.id">
          ...
          <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
            <el-table-column type="index" label="序号" width="100" align="center"></el-table-column>
            <el-table-column prop="nickname" label="动态用户名" align="center"></el-table-column>
            <el-table-column prop="cirName" label="圈子名称" align="center"></el-table-column>

            <el-table-column prop="" label="动态用户头像" align="center">
              <template slot-scope="scope">
                <el-image v-if="scope.row.dyType==2" :src="scope.row.picture"></el-image>
              </template>
            </el-table-column>
            <el-table-column prop="content" label="动态内容" align="center"></el-table-column>
            <el-table-column label="状态" align="center">
              <template slot-scope="scope">
                <span v-if="scope.row.state==0">待审核</span>
                <span v-else-if="scope.row.state==1">上架</span>
    ...
    <div v-if="type==4">
      ...
      <el-page-header @back="goBack" content="回复列表">
      </el-page-header>
      <el-divider></el-divider>
      <el-select v-model="state" placeholder="请选择类型" class="handle-search mr10">
        <el-option v-for="(item,index) in stateselect" :key="index" :label="item.name" :value="item.id"></el-option>
      ...
      <div style="display: flex;margin-top: 10px;" v-for="(el,index) in twoComment" :key="index">
        ...
        <div style="width: 100%;">
          ...
          <div style="display: flex;justify-content: flex-end;margin-top: 10px;">
            <el-button type="primary" class="handle-del mr10" @click="updateDynamicCommentState(el)">
